#!/usr/bin/env python3
"""Move the real Kinova by a small Pink IK joint target.

中文说明：该脚本会真实移动 Kinova，但只用于很小位移的 smoke test。
它先读取当前关节角，用 Pink/Pinocchio 求目标关节角，检查收敛误差和关节
变化量，最后只有在用户输入 MOVE 后才调用 Kortex PlayJointTrajectory。

This script:
1. Reads current measured Kortex joint angles.
2. Solves a small target offset with Pink/Pinocchio.
3. Performs conservative safety checks on the solved joint delta.
4. Only after explicit confirmation, executes Kortex PlayJointTrajectory.

Use this as the first real-robot smoke test for replacing Kortex
ComputeInverseKinematics with an external IK backend.
"""

from __future__ import annotations

import argparse
import sys
from pathlib import Path

import numpy as np
import yaml

PROJECT_ROOT = Path(__file__).resolve().parents[1]
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

from control.kortex_controller import KortexController  # noqa: E402
from scripts.test_pink_ik_gen3_lite import solve_pink_ik  # noqa: E402


def parse_args():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--robot-config",
        type=Path,
        default=PROJECT_ROOT / "configs" / "robot_config.yaml",
    )
    parser.add_argument(
        "--urdf",
        type=Path,
        default=PROJECT_ROOT / "kortex_description" / "robots" / "gen3_lite.urdf",
    )
    parser.add_argument(
        "--frame",
        default="tool_frame",
        help="Robot frame to control. Common options: tool_frame, end_effector_link, gripper_base_link.",
    )
    parser.add_argument(
        "--target-offset",
        type=float,
        nargs=3,
        default=[0.01, 0.0, 0.0],
        metavar=("DX", "DY", "DZ"),
        help="Small target translation offset from current URDF frame pose, in meters.",
    )
    parser.add_argument(
        "--target-position",
        type=float,
        nargs=3,
        default=None,
        metavar=("X", "Y", "Z"),
        help="Absolute target frame position in URDF/world coordinates. Prefer --target-offset for this smoke test.",
    )
    parser.add_argument(
        "--target-rpy-deg",
        type=float,
        nargs=3,
        default=None,
        metavar=("ROLL", "PITCH", "YAW"),
        help="Absolute target frame orientation as RPY degrees. If omitted, current orientation is kept.",
    )

    parser.add_argument("--dt", type=float, default=0.05)
    parser.add_argument("--iterations", type=int, default=200)
    parser.add_argument("--position-cost", type=float, default=60.0)
    parser.add_argument("--orientation-cost", type=float, default=1.0)
    parser.add_argument("--posture-cost", type=float, default=0.05)
    parser.add_argument("--position-tolerance", type=float, default=0.002)
    parser.add_argument("--orientation-tolerance", type=float, default=0.03)
    parser.add_argument("--solver", default=None)
    parser.add_argument("--list-frames", action="store_true")
    parser.add_argument("--no-wrap-current-joints", action="store_true")
    parser.set_defaults(current_joints_deg=None)

    parser.add_argument(
        "--max-joint-delta",
        type=float,
        default=15.0,
        help="Abort before motion if any solved joint delta exceeds this many degrees.",
    )
    parser.add_argument(
        "--max-position-error",
        type=float,
        default=0.004,
        help="Abort before motion if Pink final position error exceeds this many meters.",
    )
    parser.add_argument(
        "--max-orientation-error",
        type=float,
        default=0.08,
        help="Abort before motion if Pink final orientation error exceeds this many radians.",
    )
    parser.add_argument("--duration", type=float, default=8.0, help="Kortex trajectory duration in seconds.")
    parser.add_argument("--timeout", type=float, default=30.0)
    parser.add_argument(
        "--yes",
        action="store_true",
        help="Skip explicit MOVE confirmation. Use only after the script has been tested.",
    )
    return parser.parse_args()


def joint_values_deg(joint_angles):
    return [float(j.value) for j in joint_angles.joint_angles]


def load_robot_config(path: Path):
    if not path.exists():
        raise FileNotFoundError(
            f"Missing robot config: {path}\n"
            "Copy configs/robot_config.example.yaml to configs/robot_config.yaml and edit it."
        )
    with open(path, "r", encoding="utf-8") as f:
        data = yaml.safe_load(f) or {}
    if "kortex" not in data:
        raise KeyError(f"{path} must contain a 'kortex' section.")
    return data


def shortest_delta_deg(target, current):
    return [((float(t) - float(c) + 180.0) % 360.0) - 180.0 for t, c in zip(target, current)]


def safety_check(result, current_raw_deg, args):
    if result is None:
        return False
    if not result["converged"]:
        print("Safety stop: Pink did not converge.")
        return False
    if result["position_error_m"] > args.max_position_error:
        print(
            f"Safety stop: position error {result['position_error_m']:.6f} m "
            f"> {args.max_position_error:.6f} m."
        )
        return False
    if result["orientation_error_rad"] > args.max_orientation_error:
        print(
            f"Safety stop: orientation error {result['orientation_error_rad']:.6f} rad "
            f"> {args.max_orientation_error:.6f} rad."
        )
        return False

    delta = shortest_delta_deg(result["solved_joints_deg"], current_raw_deg)
    max_delta = max(abs(v) for v in delta)
    print("\nShortest deltas from measured raw Kortex joints [deg]:")
    print("  " + ", ".join(f"{v:.3f}" for v in delta))
    print(f"Max shortest joint delta: {max_delta:.3f} deg")
    if max_delta > args.max_joint_delta:
        print(f"Safety stop: max joint delta exceeds --max-joint-delta={args.max_joint_delta:.3f} deg.")
        return False
    return True


def main():
    args = parse_args()

    if args.list_frames:
        solve_pink_ik(args)
        return

    cfg = load_robot_config(args.robot_config)
    kortex = cfg["kortex"]

    print("\n========== Real Pink IK Joint Move Smoke Test ==========")
    print("This script may move the real robot after explicit confirmation.")
    print(f"Kortex IP: {kortex['ip']}")
    print(f"Target offset [m]: {args.target_offset}")
    print(f"Trajectory duration [s]: {args.duration:.3f}")

    with KortexController(
        ip=str(kortex["ip"]),
        username=str(kortex["username"]),
        password=str(kortex["password"]),
        port=int(kortex["port"]),
    ) as robot:
        robot.clear_faults()
        robot.set_single_level_servoing()
        measured = robot.get_measured_joint_angles()
        current_raw = joint_values_deg(measured)

        print("\nMeasured Kortex joint angles [deg], raw:")
        print("  " + ", ".join(f"{v:.3f}" for v in current_raw))

        result = solve_pink_ik(args, current_joints_deg=current_raw)
        if not safety_check(result, current_raw, args):
            print("No motion command sent.")
            return

        solved = result["solved_joints_deg"]
        print("\nKortex target joint angles [deg]:")
        print("  " + ", ".join(f"{v:.3f}" for v in solved))
        print("\nThis will execute PlayJointTrajectory to the Pink IK joint target.")
        if not args.yes:
            answer = input("Type MOVE to execute this small joint move: ").strip()
            if answer != "MOVE":
                print("Confirmation not received. No motion command sent.")
                return

        target_joint_angles = robot.make_joint_angles(solved)
        ok = robot.move_to_joint_angles(
            target_joint_angles,
            name="pink_ik_smoke",
            timeout_s=float(args.timeout),
            duration_s=float(args.duration),
        )

        after = robot.get_measured_joint_angles()
        after_values = joint_values_deg(after)
        print("\nAfter Kortex joint angles [deg], raw:")
        print("  " + ", ".join(f"{v:.3f}" for v in after_values))
        print("Move result:", "success" if ok else "aborted/failed")


if __name__ == "__main__":
    main()
